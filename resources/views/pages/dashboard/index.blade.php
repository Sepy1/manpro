...
@section('admin-content')
    ...
    <div
        ...
        class="text-[15px] leading-relaxed md:text-base"
    >
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-4">
        <button type="button" @click="openStatusModal('planned', 'Planned')" class="rounded-2xl border border-gray-200 bg-white p-4 text-left dark:border-gray-800 dark:bg-white/[0.03]">
            <p class="text-sm font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-400">Planned</p>
            <p class="mt-2 text-3xl font-bold text-gray-800 dark:text-white/90">{{ $projectsByStatus['planned'] }}</p>
        </button>
        <button type="button" @click="openStatusModal('in_progress', 'In Progress')" class="rounded-2xl border border-yellow-200 bg-yellow-50/60 p-4 text-left dark:border-yellow-800/60 dark:bg-yellow-900/10">
            <p class="text-sm font-semibold uppercase tracking-wide text-yellow-700 dark:text-yellow-300">In Progress</p>
            <p class="mt-2 text-3xl font-bold text-yellow-800 dark:text-yellow-200">{{ $projectsByStatus['in_progress'] }}</p>
        </button>
        <button type="button" @click="openStatusModal('completed', 'Completed')" class="rounded-2xl border border-green-200 bg-green-50/60 p-4 text-left dark:border-green-800/60 dark:bg-green-900/10">
            <p class="text-sm font-semibold uppercase tracking-wide text-green-700 dark:text-green-300">Completed</p>
            <p class="mt-2 text-3xl font-bold text-green-800 dark:text-green-200">{{ $projectsByStatus['completed'] }}</p>
        </button>
        <button type="button" @click="openStatusModal('delayed', 'Delayed')" class="rounded-2xl border border-red-200 bg-red-50/60 p-4 text-left dark:border-red-800/60 dark:bg-red-900/10">
            <p class="text-sm font-semibold uppercase tracking-wide text-red-700 dark:text-red-300">Delayed</p>
            <p class="mt-2 text-3xl font-bold text-red-800 dark:text-red-200">{{ $projectsByStatus['delayed'] }}</p>
        </button>
    </div>
    ...
    <div class="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-3">
        <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] xl:col-span-2">
            <div class="mb-3 flex items-center justify-between">
                <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Todo Project & Step</h3>
                <span class="text-sm text-gray-500 dark:text-gray-400">Step belum completed</span>
            </div>
                ...
                @forelse ($todoProjects as $project)
                    ...
                    <div class="rounded-xl border border-gray-200 p-3 dark:border-gray-700">
                        <div class="flex items-center gap-3">
                            <button
                                type="button"
                                class="min-w-0 flex-1 truncate text-left text-base font-semibold text-gray-800 hover:underline dark:text-white/90"
                            ...
                            <div class="w-32 shrink-0">
                                ...
                                <p class="mt-1 text-right text-xs text-gray-500 dark:text-gray-400">{{ $todoProgress }}%</p>
                            </div>
                        </div>
                        <ul class="mt-2 space-y-1">
                            @foreach ($project->steps as $step)
                                <li class="flex items-start justify-between gap-2 text-base">
                                    <button
                                        type="button"
        ...
        <div>
            <div class="flex h-full flex-col rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Step terlambat: <span class="font-semibold text-red-600 dark:text-red-400">{{ $overdueSteps }}</span>
                </p>
                <div class="mt-3 flex-1 space-y-2 overflow-y-auto pr-1">
                    @forelse ($upcomingDeadlines as $step)
                        <div class="rounded-lg border border-gray-200 px-3 py-2 dark:border-gray-700">
                            <button
                                type="button"
                                class="text-left text-base font-medium text-gray-800 hover:underline dark:text-white/90"
                                @click="openTodoStepModal($el)"
                                data-action="{{ route('admin.daftar-project.step.update', $step) }}"
                                data-step-name="{{ $step->step_name }}"
                                data-start-date="{{ optional($step->start_date)->format('Y-m-d') }}"
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ $step->project?->name }}</p>
