...
@section('admin-content')
    <x-common.page-breadcrumb pageTitle="Manajemen Divisi" />
    ...
    <div class="space-y-4" x-data="{ showAddForm: false }">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <form x-show="showAddForm" x-cloak method="POST" action="{{ route('admin.manajemen-divisi.store') }}" class="mb-4 grid grid-cols-1 gap-3 rounded-xl border border-gray-200 p-3 md:grid-cols-4 dark:border-gray-700">
                @csrf
                <div class="md:col-span-2">
                    <input type="text" name="name" value="{{ old('name') }}" required placeholder="Nama Divisi"
                ...
                <table class="min-w-full table-fixed border-collapse">
                    <thead>
                        <tr class="border-b border-gray-200 dark:border-gray-800">
                            ...
                            <th class="w-[40%] px-3 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Nama Divisi</th>
                    ...
                    <tbody>
                        @forelse ($divisions as $division)
                            <tr x-data="{ edit: false }" class="border-b border-gray-100 dark:border-gray-800">
                                <td class="px-3 py-3 text-sm text-gray-700 dark:text-gray-300">{{ $division->id }}</td>
                                <td class="px-3 py-3 text-sm text-gray-700 dark:text-gray-300">
                                    <span x-show="!edit">{{ $division->name }}</span>
                                    <form x-show="edit" x-cloak method="POST" action="{{ route('admin.manajemen-divisi.update', $division) }}" class="flex items-center gap-2">
                                        @csrf
                                        @method('PUT')
                                        <input type="text" name="name" value="{{ $division->name }}" required class="h-9 w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm dark:border-gray-700 dark:text-white/90" />
                                        <label class="inline-flex items-center gap-1 text-xs text-gray-600 dark:text-gray-300">
                                            <input type="checkbox" name="is_active" value="1" @checked($division->is_active) class="rounded border-gray-300 dark:border-gray-600" />
                                            Aktif
                                        </label>
                                ...
                                <td class="px-3 py-3 text-sm">
                                    <span class="inline-flex rounded-full px-2 py-0.5 text-xs font-medium {{ $division->is_active ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300' : 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300' }}">
                                ...
                                <td class="px-3 py-3 text-sm">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <form method="POST" action="{{ route('admin.manajemen-divisi.delete', $division) }}" onsubmit="return confirm('Hapus divisi ini?')">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-3 py-8 text-center text-sm text-gray-500 dark:text-gray-400">Belum ada divisi.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
