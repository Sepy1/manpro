...
@section('admin-content')
    <x-common.page-breadcrumb pageTitle="Parameter: Aplikasi CR Eksternal" />
    ...
    <div class="space-y-4" x-data="{ showAddForm: false }">
        ...
        <div class="content-card p-5">
            <div class="mb-4 flex items-center justify-between">
                <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Daftar aplikasi</h3>
            ...
            <form x-show="showAddForm" x-cloak method="POST" action="{{ route('admin.parameter.cr-aplikasi.store') }}"
                class="mb-4 grid grid-cols-1 gap-3 rounded-xl border border-gray-200 p-3 md:grid-cols-6 dark:border-gray-700">
                @csrf
                <div class="md:col-span-3">
                    <input type="text" name="name" value="{{ old('name') }}" required placeholder="Nama aplikasi/sistem"
                ...
                <div class="md:col-span-1">
                    <input type="number" name="sort_order" value="{{ old('sort_order', 0) }}" min="0" max="65535" placeholder="Urut"
                        class="h-10 w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm dark:border-gray-700 dark:text-white/90" />
                </div>
                        ...
                        @forelse ($items as $item)
                            <tr x-data="{ edit: false }" class="border-b border-gray-100 dark:border-gray-800">
                                ...
                                <td class="px-3 py-3 text-sm text-gray-700 dark:text-gray-300">
                                    ...
                                    <form x-show="edit" x-cloak method="POST" action="{{ route('admin.parameter.cr-aplikasi.update', $item) }}" class="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-2">
                                        @csrf
                                        @method('PUT')
                                        ...
                                        <input type="number" name="sort_order" value="{{ $item->sort_order }}" min="0" max="65535" title="Urutan sortir"
                                ...
                                <td class="px-3 py-3 text-sm">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <form method="POST" action="{{ route('admin.parameter.cr-aplikasi.delete', $item) }}" onsubmit="return confirm('Hapus aplikasi ini?')">
                                            @csrf
                                            @method('DELETE')
